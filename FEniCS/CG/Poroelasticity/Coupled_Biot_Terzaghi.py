...
from ufl_legacy import nabla_div, nabla_grad, elem_op
import numpy as np
from ufl_legacy.tensors import as_matrix
import  sys
import meshio
import mshr
...
H=1
D=0.5
mesh=RectangleMesh(Point(0, 0.0), Point(float(D/2),-H), int(D*30), abs(int(H*60)),"crossed")
...
for pot in range(steps):
    ...
    # A1 = cupyx.scipy.sparse.csr_matrix(A1)
...
plt.savefig('resultados/consolidacion_dt%s_gpu_%s.png'%(round(dtdot,5),scheme),dpi=300)
plt.close()
ig, ax = plt.subplots()
...
plt.savefig('resultados/disp_dt%s_gpu_%s.png'%(round(dtdot,5),scheme),dpi=300,)
plt.close()

# L2=np.array(L2)
# plt.semilogy(L2[:,1],L2[:,0])
# plt.savefig('resultados/L2norm_dt%s_gpu_%s.png'%(round(dtdot,5),scheme),dpi=300)
