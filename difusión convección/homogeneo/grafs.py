...
if type_plot == "t-L2":
    ...
    plot = error.plot(x='tiempo',xlim=[0,math.pi/2],ylim=[1e-5,2],title=f"Error convergence for dt = {dt}",logy=True,grid=True,xlabel='time',ylabel='L2 norm')
    plt.grid(True,which="both",alpha=0.3, ls='-')
    plt.savefig(f'L2norm_CFL_{dt}.png',dpi=300)
elif type_plot == "CFL-sumL2":
    error=pd.DataFrame()
    it = 0
    for scheme in types:
        ...
        if it == 0:
            error = pd.concat([error,df1[scheme],df1['max_CFL_number']],sort=True,axis=1)
        else:            
            error = pd.concat([error,df1[scheme]],sort=True,axis=1)
        it += 1
    print(error)
    plot = error.plot(x='max_CFL_number',title=f"Error convergence ",loglog=True,grid=True,xlabel='time',ylabel='L2 norm')
    plt.grid(True,which="both",alpha=0.3, ls='-')
    plt.xlabel("$max(CFL)$ ",fontsize=15)
    plt.ylabel("$L^2$",fontsize=15)
    plt.savefig(f'CFL-L2.png',dpi=300)
